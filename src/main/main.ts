...
ipcMain.handle('submitToChatGPT', async (e, text: string) => {
  ...
  try {
    ...
    const prompt = endent`Answer the following user query. Below are internet sources to help you answer. Cite sources as [1] or [2] or [3] after each sentence (not just the very end) to back up your answer (Ex: Correct: [1], Correct: [2][3], Incorrect: [1, 2]).:
